...
class RIG_DEBUGGER_OT_MirrorCustomBones(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        return context.active_object is not None and ob_type == "ARMATURE"
    ...
    def execute(self, context):
        ...
        def mirror_custom_bone(ob, pose_bone1, pose_bone2):
            ...
            pose_bone2.custom_shape = pose_bone1.custom_shape # Custom Bone Shape
            pose_bone2.custom_shape_scale = pose_bone1.custom_shape_scale # Bone Scale
            pose_bone2.use_custom_shape_bone_size = pose_bone1.use_custom_shape_bone_size # Use Bone Size

            data_bones[pose_bone2.name].show_wire = data_bones[pose_bone1.name].show_wire # Wire
            # .custom_shape
            # .custom_shape_scale
        ...
        if active_bone is not None:
            active_bone_side = str(getDirection(active_bone.name, flip=False) ) # String
            ...
            if active_bone_side == "l" or active_bone_side == "r" :
                pose_bones = context.object.pose.bones
                for i in pose_bones:
                    bone_side = getDirection(i.name, flip=False)
                    ...
                    if bone_side == active_bone_side:
                        ...
                        if flipped_name is not None:
                            pose_bone_gotten = pose_bones.get(flipped_name)

                            # If there is an opposite flipped named bone
                            if pose_bone_gotten is not None:
                                #print(pose_bone_gotten.name)
                                mirror_custom_bone(active_object, i, pose_bone_gotten)
                                total_bones_affected += 1
                    else:
                        continue
                reportString = "Mirrored %d/%d of Selected Bones out of %s" % (total_bones_affected, total_selected_bones, total_bones)
            else:
...
class REGEX_SCANNER_MT_DropdownMenu1(bpy.types.Menu):
    bl_idname = "REGEX_SCANNER_MT_DropdownMenu1"
...
class REGEX_SCANNER_MT_DropdownMenu2(bpy.types.Menu):
    bl_idname = "REGEX_SCANNER_MT_DropdownMenu2"
...
class VertexGroupsOpsDraw:
    ...
    def draw(self, context):
        ...
        #Dropdown Menu
        row = col.row(align=True)
        row.menu("REGEX_SCANNER_MT_DropdownMenu2", icon="DOWNARROW_HLT", text="")
        ...
        #Dropdown Menu
        row = col.row(align=True)
        row.menu("REGEX_SCANNER_MT_DropdownMenu1", icon="DOWNARROW_HLT", text="")
